...
\begin{document}
	\title{\huge \team \\ Technology Review \\ CS 462 Winter 2017}
	\author{\large \name \\ \grp}
	...
	Church Community Builder works well because they are already using this software for other parts of their website. 
	However, they are not currently using Church Community Builder for their bulletin page on the website which would mean they would have to change how they are presenting the bulletins on their website, or they would have to update the bulletins in two different places. 
	...
	Recently, the church has completely overhauled their website and are now using Wordpress. 
	If we are granted access to their website, we can use REST API from Wordpress to directly pull the content from the Bulletin page on the website to the Bulletin page on the app. 

	\subsection{Selection}
	For this project, the option that will work best is Wordpress. 
	Because the church has recently changed their website to use Wordpress, we can use REST API from Wordpress free of charge. 
	We will need to enable REST API on their website and then we can pull the information directly from the website.
	This will allow the web development team to only have to update the bulletin page as normal on their website and the changes will be reflected on the app as well. 
	...
	With the church's updated website, they are now using Wordpress so we will make use of Wordpress API for the e-bulletins.
